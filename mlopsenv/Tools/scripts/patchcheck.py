...
def get_git_remote_default_branch(remote_name):
    ...
    try:
        remote_info = subprocess.check_output(
            cmd, stderr=subprocess.DEVNULL, cwd=SRCDIR, encoding="UTF-8", env=env
        )
    except subprocess.CalledProcessError:
        return None
    ...
    for path in file_paths:
        ...
        try:
            with open(abspath, "rb") as f:
                lines = f.readlines()
            new_lines = [ws_re.sub(rb"\1", line) for line in lines]
            if new_lines != lines:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ci", help="Perform pass/fail checks")
