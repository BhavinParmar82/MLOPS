        ...
        for key in sorted(family_stats):
            ...
            rows.append(
                (
                    ...
                    f"{100*family_stats[key]/total:0.1f}%",
                )
            )
        ...
        for value, index in failures:
            ...
            rows.append(
                (
                    ...
                    value,
                    f"{100*value/total_failures:0.1f}%",
                )
            )
...
def categorized_counts(opcode_stats):
    ...
    specialized_instructions = {
        op
        for op in opcode._specialized_instructions
        if "__" not in op and "ADAPTIVE" not in op
    }
    adaptive_instructions = {
        op for op in opcode._specialized_instructions if "ADAPTIVE" in op
    }
        ...
        emit_table(
            ...
            (
                ...
                (
                    "Not specialized",
                    not_specialized,
                    f"{not_specialized*100/total:0.1f}%",
                ),
...
def emit_pair_counts(opcode_stats, total):
    ...
    with Section(
        ...
        summary="Top 3 predecessors and successors of each opcode",
    ):
        ...
        for name, i in opmap.items():
            ...
            if total1:
                pred_rows = [
                    (opname[pred], count, f"{count/total1:.1%}")
                    for (pred, count) in predecessors[i].most_common(3)
                ]
            if total2:
                succ_rows = [
                    (opname[succ], count, f"{count/total2:.1%}")
