...
def fetch_server_certificate(host, port):
    ...
    def strip_to_x509_cert(certfile_contents, outfile=None):
        ...
        if not m:
            ...
            try:
                ...
                status, output = subproc(r'openssl x509 -in "%s" -out "%s"' % (tn, tn2))
                if status != 0:
                    raise RuntimeError(
    ...
    if status != 0:
        raise RuntimeError(
            "OpenSSL connect failed with status %s and " "output: %r" % (status, output)
        )
    certtext = strip_to_x509_cert(output)
    if not certtext:
        raise ValueError(
            "Invalid response received from server at %s:%s" % (host, port)
        )
    return certtext
