...
def regen_frozen(modules, frozen_modules: bool):
    ...
    for mod in modules:
        ...
        if not frozen_modules:
            line = ('{"%s", NULL, 0, %s, GET_CODE(%s)},') % (mod.name, pkg, code_name)
        else:
...
def regen_makefile(modules):
    ...
    for src in _iter_sources(modules):
        ...
        freeze = f"{freezecmd} {src.frozenid} " f"$(srcdir)/{pyfile} {frozen_header}"
        rules.extend(
            [
...
def regen_pcbuild(modules):
    ...
    corelines.append(
        f'    <ClCompile Include="..\\Python\\deepfreeze\\deepfreeze.c" />'
    )
